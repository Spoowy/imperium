...
/*
	CASL special keywords is the action 'manage' and the entity 'all'.
*/
...
export function defineRulesFor(user: User | null): RawRuleOf<Ability<AppAbilityTuple>>[] {
	...
	if (user?.id) {
		...
		// can('read', 'Photo', ['id', 'name', 'public', 'categories.*', 'owner.*'], {public: true}); // Can read public photos
		can('read', 'Photo', ['id', 'name', 'public'], {public: true}); // Can read public photos
